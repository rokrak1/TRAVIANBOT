...
export const fetchOasis = async (
  ...
  await page.logger(
    LoggerLevels.INFO,
    `Available positions: ${positions.map((p) => `[${p.x},${p.y}]`).join(", ")}`
  );
  ...
  const sortedOasis = sortOasesByChebyshevDistance(
    { x: configuration.startX, y: configuration.startY },
    allOasis
  );
  ...
  const filterOutSameOasis = sortedOasis.filter(
    (oasis, index, self) =>
      ...
      self.findIndex(
        (t) =>
          t.position.x === oasis.position.x && t.position.y === oasis.position.y
      )
  );
  ...
  const richOasis = filterOutSameOasis.filter((oasis) => {
    ...
    const hasInvalidAnimal = animals.some(
      (animal) => !validAnimals.includes(animal)
    );
    ...
    const hasRequiredAnimal = mustIncludeAnimals.every((animal) =>
